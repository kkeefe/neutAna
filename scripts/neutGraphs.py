...
def makeGraphs(tf_dict, tdirs, zdirs, output_dir, graph_name, output_name, x_axis_title=None, y_axis_title=None, lepPdg=None, isFHC=None, saveGraphs=None):
    ...
    if isinstance(asic_ints[0], ROOT.TH1):
        tmg = make_integral_hist(asic_ints, output_name, tdirs, zdirs, lepPdg, isFHC)
        if saveGraphs is not None:
...
def readRootDataFile(infile, file_dir="test", lepPdg=None, isFHC=None):
    ...
    for i, (theta_dir, td) in enumerate(zip(theta_dirs, tdir)):
        sg = True if i == 0 else None
        makeGraphs(tf_dict, theta_dir, zpos_dirs, td, "hTile",  "tile_cZpos", "Tile Resets", "Counts", lepPdg, isFHC, saveGraphs="Tile" if sg else None)
        makeGraphs(tf_dict, theta_dir, zpos_dirs, td, "hAsic",  "asic_cZpos", "ASIC Resets", "Counts", lepPdg, isFHC, saveGraphs="ASIC" if sg else None)
        makeGraphs(tf_dict, theta_dir, zpos_dirs, td, "hPixel", "pixel_cZpos",  "Pixel Resets", "Counts")
        makeGraphs(tf_dict, theta_dir, zpos_dirs, td, "tgLepKEAsic", "LepKE_AsicResets_cZpos",  "LepKE", "Max ASIC Resets", lepPdg, isFHC, saveGraphs="ASIC_lepKE" if sg else None)
        makeGraphs(tf_dict, theta_dir, zpos_dirs, td, "tgLepKETile", "LepKE_TileResets_cZpos",  "LepKE", "Max Tile Resets", lepPdg, isFHC, saveGraphs="Tile_lepKE" if sg else None)

    # control for zpos graphs
    for zpos_dir, zd in zip(zpos_dirs, zdir):
        makeGraphs(tf_dict, theta_dirs, zpos_dir, zd, "hTile",  "tile_cTheta", "Tile Resets", "Counts")
        makeGraphs(tf_dict, theta_dirs, zpos_dir, zd, "hAsic",  "asic_cTheta", "ASIC Resets", "Counts")
        makeGraphs(tf_dict, theta_dirs, zpos_dir, zd, "hPixel", "pixel_cTheta",  "Pixel Resets", "Counts")
        makeGraphs(tf_dict, theta_dirs, zpos_dir, zd, "tgLepKEAsic", "LepKE_AsicResets_cTheta",  "LepKE", "Max ASIC Resets")
        makeGraphs(tf_dict, theta_dirs, zpos_dir, zd, "tgLepKETile", "LepKE_TileResets_cTheta",  "LepKE", "Max Tile Resets")
...
def readFeatherDataFile(output_name, input_file="./scripts/neutMP60k.feather", size=16):
    ...
    for i, route in enumerate(["snake", "left", "trunk"]):
    ...
    ax.set_xlabel("Remote Buffer Depth")
    ax.set_ylabel("Local Buffer Depth")
    plt.tight_layout()
    ...
    ax.hist(stack_data, bins=200, range=(1, 3001), histtype='bar', stacked=True, log=False, label=routes)
    ax.set_ylabel("10 events / remote trans avg.")
    ...
    trans_data = [frq, f"{size}", remote_trans_avg[-1], remote_trans_avg[0], remote_trans_avg[0]/5, remote_trans_avg[1], remote_trans_avg[1]/5, remote_trans_avg[2], remote_trans_avg[2]/5]
...
def main():
    ...
    neut_buff_tab = latextable.draw_latex(table_neut_buf_data, caption="Integral Data", label="tab:sum")
    saveTable(neut_buff_tab, table_neut_buf_file)

    # # vertex analysis
    # readRootDataFile(infile=".\\pdfs\\electron_nu_fhc_graphs.root", file_dir="fhc_pdg12")
